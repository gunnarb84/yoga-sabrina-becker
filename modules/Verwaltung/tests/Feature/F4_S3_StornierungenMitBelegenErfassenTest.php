...
it('creates a credit note when cancelling a paid transfer registration', function (): void {
    ...
    expect($creditNote->getAttribute('rechnung_id'))->toBe($invoice->getAttribute('id'));
...
it('creates a cash return when cancelling a paid cash registration', function (): void {
    ...
    $payment = Payment::whereRaw('anmeldung_id = ?', [Uuid::fromString($registration->value()->registrationId)->getBytes()])->first();
    $receipt = CashReceipt::whereRaw('zahlung_id = ?', [Uuid::fromString($payment->getAttribute('id'))->getBytes()])->first();
    ...
    expect($cashReturn->getAttribute('bareinnahmenbeleg_id'))->toBe($receipt->getAttribute('id'));
});
...
it('increments reversal document numbers for consecutive cancellations', function (): void {
    ...
    $firstPayment = Payment::whereRaw('anmeldung_id = ?', [Uuid::fromString($firstRegistration->value()->registrationId)->getBytes()])->first();
    $firstInvoice = Invoice::whereRaw('zahlung_id = ?', [Uuid::fromString($firstPayment->getAttribute('id'))->getBytes()])->first();
    $secondPayment = Payment::whereRaw('anmeldung_id = ?', [Uuid::fromString($secondRegistration->value()->registrationId)->getBytes()])->first();
    $secondInvoice = Invoice::whereRaw('zahlung_id = ?', [Uuid::fromString($secondPayment->getAttribute('id'))->getBytes()])->first();

    $mark = new MarkTransferPaid();
    $mark->execute(new MarkTransferPaidRequest(invoiceId: $firstInvoice->getAttribute('id')));
    $mark->execute(new MarkTransferPaidRequest(invoiceId: $secondInvoice->getAttribute('id')));

    $cancel = new CancelRegistration(app(NextNumber::class));
    $cancel->execute(new CancelRequest(registrationId: $firstRegistration->value()->registrationId));
    $cancel->execute(new CancelRequest(registrationId: $secondRegistration->value()->registrationId));
