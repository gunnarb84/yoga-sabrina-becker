...
namespace Yoga\Modules\Webseite\Application\Activity\ActivityOverview;
...
final readonly class ActivityOverviewQuery
{
    ...
    public function execute(?string $typeFilter = null): array
    {
        ...
        $query = DB::table('verwaltung_aktivitaeten')
            ->select([
                'id',
                'slug',
                'typ',
        ...
        $rows = $query->orderBy('titel')->get();

        $items = $rows->map(function (object $row) use ($now): object {
